...
def test_mh_func():
    ...
    init_params = {"beta": np.zeros(dim), "intercept": 0.0}
    mcmc.run(random.PRNGKey(0), data=data, labels=labels, init_params=init_params)
...
def test_beta_bernoulli_x64():
    ...
    kernel = MH(model, proposal_var=0.001)
...
#     kernel = kernel_cls(model, proposal_var=0.01)
#     mcmc = MCMC(kernel, warmup_steps, num_samples, progress_bar=False)
#     init_params = {"coeffs": np.zeros(dim)}
#     mcmc.run(random.PRNGKey(2), labels=labels, init_params=init_params)
#     mcmc.print_summary()
#     samples = mcmc.get_samples()
