...
def launch(params_dict):
    ...
    print(params_dict)
    save_params("examples/params/task_beta_params.yaml", params_dict)

    filter_ext = (".pyc", ".log", ".git", ".mp4", ".npz", ".ipynb", ".png")
    ...
    launch_api.run_command(
        command="bash /dar_payload/rdb/examples/cloud/run_task_beta.sh",
...
if __name__ == "__main__":
    ...
    # ========== Parameters =============
    template = load_params("examples/params/task_beta_template.yaml")

    if LOCAL_MODE:
        launch(template)
    else:
        params = {"RANDOM_KEYS": list(range(4))}
