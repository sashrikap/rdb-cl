...
class HighwayDriveWorld_Week3(HighwayDriveWorld):
    def __init__(
        ...
        self._car1_range = jnp.arange(car1_range[0], car1_range[1], car_delta)
        self._car2_range = jnp.arange(car2_range[0], car2_range[1], car_delta)
    ...
    def _get_nonlinear_features_dict(self, feats_dict):
        ...
        nlr_feats_dict["dist_cars"] = compose(
            ...
            partial(
                ...
                sigma=jnp.array([self._car_width / 2, self._car_length]),
            ),
        )
        # Gaussian
        ...
        """nlr_feats_dict["dist_lanes"] = compose(
            jnp.sum, quadratic_feat, partial(item_index_feat, index=self._goal_lane)
        )"""
        # Quadratic barrier function
        """nlr_feats_dict["dist_fences"] = compose(
            jnp.sum, partial(sigmoid_feat, mu=8.0 / self._car_width), neg_feat
        )"""
        nlr_feats_dict["dist_fences"] = compose(
            jnp.sum,
        ...
        """nlr_feats_dict["dist_fences"] = compose(
            ...
            partial(
                feature.gaussian_feat,
                sigma=jnp.array([self._car_width / 3, self._car_length / 3]),
            ),
            # debug_print
        )"""
        bound = self._control_bound
        nlr_feats_dict["control"] = compose(jnp.sum, feature.quadratic_feat)
        nlr_feats_dict["speed"] = compose(
            jnp.sum, partial(feature.quadratic_feat, goal=self._goal_speed)
        )
...
class Week3_01(HighwayDriveWorld_Week3):
    def __init__(self):
        ...
        num_lanes = 3
        ## Weird 1
        # car1 = jnp.array([0.0, 0.45, jnp.pi / 2, 0])
        # car2 = jnp.array([-0.125, 0.4, jnp.pi / 2, 0])
        ## Weird 2
        # car1 = jnp.array([0.0, 0.45, jnp.pi / 2, 0])
        # car2 = jnp.array([-0.125, 0.4, jnp.pi / 2, 0])
