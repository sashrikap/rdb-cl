from rdb.infer.algos import *


class PGM(object):
    """Generic Probabilisitc Graphical Model Class.

    Features:
        * Has built-in sampler.

    Methods:
        likelihood (fn): p(obs | theta) p(theta)

    """

    def __init__(
        self, rng_key, kernel, prior, proposal, sample_method="mh", sample_args={}
    ):
        self._rng_key = rng_key
        self._sampler = self._build_sampler(
            kernel, prior, proposal, sample_method, sample_args
        )

    def update_key(self, rng_key):
        self._rng_key = rng_key
        # self._sampler.update_key(rng_key)

    def _build_sampler(self, kernel, prior, proposal, sample_method, sample_args):
        if sample_method == "mh":
            return MetropolisHasting(
                self._rng_key,
                kernel=kernel,
                prior=prior,
                proposal=proposal,
                **sample_args
            )
        else:
            raise NotImplementedError
