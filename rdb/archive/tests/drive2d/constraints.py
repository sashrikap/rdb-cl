...
def test_overspeed():
    ...
    assert np.all(constr_overspeed(env.state, action))

    constr_overspeed = partial(is_overspeed, env=env, max_speed=0.8)
    assert np.all(not constr_overspeed(env.state, action))
...
def test_underspeed():
    ...
    assert np.all(not constr_underspeed(env.state, action))

    constr_underspeed = partial(is_underspeed, env=env, min_speed=0.8)
    assert np.all(constr_underspeed(env.state, action))
...
def test_wronglane():
    ...
    assert np.all(not constr_wronglane(env.state, action))

    constr_wronglane = partial(is_wronglane, env=env, lane_idx=1)
    assert np.all(constr_wronglane(env.state, action))
...
def test_uncomfortable():
    ...
    assert np.all(not constr_uncomfortable(env.state, np.zeros(2)))
    assert np.all(constr_uncomfortable(env.state, np.ones(2) * 2.0))
...
def test_collision():
    ...
    assert np.all(not constr_collision(state_front, np.zeros(2)))
    state_front[9] -= 0.1
    env.set_init_state(state_front)
    env.reset()
    assert np.all(constr_collision(state_front, np.zeros(2)))

    env.set_init_state(state_side)
    env.reset()
    constr_collision = partial(is_collision, env=env)
    assert np.all(not constr_collision(state_side, np.zeros(2)))
    state_side[8] -= 0.1
    env.set_init_state(state_side)
    env.reset()
    assert np.all(constr_collision(state_side, np.zeros(2)))
...
def test_offtrack():
    ...
    assert np.all(not constr_offtrack(state_left, np.zeros(2)))
    state_left[8] -= 0.1
    env.set_init_state(state_left)
    env.reset()
    assert np.all(constr_offtrack(state_left, np.zeros(2)))

    env.set_init_state(state_right)
    env.reset()
    constr_offtrack = partial(is_offtrack, env=env)
    assert np.all(not constr_offtrack(state_right, np.zeros(2)))
    state_right[8] += 0.1
    env.set_init_state(state_right)
    env.reset()
    assert np.all(constr_offtrack(state_right, np.zeros(2)))
